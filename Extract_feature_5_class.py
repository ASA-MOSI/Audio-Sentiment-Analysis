...
class Extract_feature:
    ...
    hop_length = None
    genre_list = ['negative', 'positive', 'neutral', 'strong_negative', 'strong_positive']
    ...
    def extract_audio_features(self, list_of_audiofiles):
        ...
        for l in target:
            if l == "negative":
                ...
                c += 1
        print("negative: %d, neutral: %d, positive: %d, strong_negative: %d, strong_positive: %d" % (a, b, c, d, e))
    ...
    def load_target(self, target):
        ...
        for i in range(0, 2199):
            if all_label_list[i][4] > 2:
                target.append("strong_positive")
            elif all_label_list[i][4] < -2:
                target.append("strong_negative")
            elif all_label_list[i][4] > 1 and all_label_list[i][4] <= 2:
                target.append("positive")
            elif all_label_list[i][4] > -2 and all_label_list[i][4] <= 1:
