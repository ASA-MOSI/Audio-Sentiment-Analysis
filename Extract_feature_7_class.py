...
class Extract_feature:
    ...
    hop_length = None
    genre_list = ['negative', 'positive', 'neutral', 'strong_negative', 'strong_positive', 'week_negative',
                  'week_positive']
    ...
    def extract_audio_features(self, list_of_audiofiles):
        ...
        for l in target:
            if l == "negative":
                ...
                b += 1
            elif l == "strong_negative":
                d += 1
            elif l == "strong_positive":
                e += 1
            elif l == "week_negative":
                f += 1
            elif l == "week_positive":
                g += 1
            else:
                c += 1
        print("negative: %d, neutral: %d, positive: %d, strong_negative: %d, strong_positive: %d,"
              " week_negative: %d,  week_positive: %d" % (a, b, c, d, e, f, g))
    ...
    def load_target(self, target):
        ...
        for i in range(0, 2199):
            if all_label_list[i][4] > 2.5:
                target.append("strong_positive")
            elif all_label_list[i][4] < -2.5:
                target.append("strong_negative")
            elif all_label_list[i][4] > 1 and all_label_list[i][4] <= 2.5:
                target.append("positive")
            elif all_label_list[i][4] > -2.5 and all_label_list[i][4] <= -1:
                target.append("negative")
            elif all_label_list[i][4] > 0.5 and all_label_list[i][4] <= 1:
                target.append("week_positive")
            elif all_label_list[i][4] > -1 and all_label_list[i][4] <= -0.5:
                target.append("week_negative")
            else:
